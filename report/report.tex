...
def __init__(self, id: int, moveTime: float, type: EventType, queue_name: str, server_id: int):
	...
	\noindent Once  we obtain the results of the fractional factorial DoE, next we analyze the main effects and interactions by performing the yates algorithm, table \ref{tab:effects}.
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{cccccc|l}
			...
			765.000 & -133.125  & 77.625    & -320.250  & 4 & -80.063  & C (ABC) \\
			\bottomrule
		\end{tabular}
	...
	Analysis of the fractional factorial design matrix (Table~\ref{tab:effects}) reveals the individual factor main effects and structural two-way interactions governing the block validation pipeline. \\
	
	\noindent The grand simulation baseline yields an average throughput of $\bar{X} = 755.906$ validated blocks per execution window. Factor $B$ (\textit{Peers}) exerts the most dominant positive main effect ($+162.938$), empirically demonstrating that expanding peer availability significantly eases processing constraints. Conversely, Factor $A$ (\textit{Block Difficulty}) and Factor $D$ (\textit{Block Interarrival Rate}) present severe negative main effects of $-121.875$ and $-80.063$, respectively. High levels in either parameter cause severe upstream queuing delays that choke cumulative throughput. \\
	
	\noindent Crucially, the analysis reveals a massive positive interaction element, $AB$ (\textit{Block Difficulty $\times$ Peers} = $+118.875$). This high-magnitude interaction establishes that the throughput degradation caused by elevated cryptographic difficulty is heavily cushioned and mitigated when the network operates at a high peer capacity. In contrast, the minor significance of Factor $C$ ($-20.625$) and interaction $BC$ ($+9.000$) indicates that block data alterations do not represent a primary operational bottleneck within this specific architecture.
	...
	\noindent Since the results of all p-values > 0.05, we fail to reject the hypothesis of similarity. So the simulation engine implemented seems to be adequate for task scheduling. The results can be checked by executing 'python3 main.py' with the same parameters and then 'python3 model\_validation'.
	% =============================================================================
